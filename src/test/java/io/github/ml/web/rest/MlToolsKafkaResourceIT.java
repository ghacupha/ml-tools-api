package io.github.ml.web.rest;

import io.github.ml.config.KafkaProperties;
...
class MlToolsKafkaResourceIT {
    ...
    void setup() {
        ...
        MlToolsKafkaResource kafkaResource = new MlToolsKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/ml-tools-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/ml-tools-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
