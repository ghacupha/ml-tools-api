...
public class  LogisticRegressionClassifierFactory<L,F> implements ClassifierFactory<L, F, LogisticRegressionClassifier<L,F>>  {
    ...
    public LogisticRegressionClassifier<L,F> trainClassifier(GeneralDataset<L, F> data) {
        return trainClassifier(data, 0.0);
    }

    public LogisticRegressionClassifier<L,F> trainClassifier(GeneralDataset<L, F> data, LogPrior prior, boolean biased) {
        return trainClassifier(data, 0.0, 1e-4, prior, biased);
    }

    public LogisticRegressionClassifier<L,F> trainClassifier(GeneralDataset<L, F> data, double l1reg) {
        return trainClassifier(data, l1reg, 1e-4);
    }

    public LogisticRegressionClassifier<L,F> trainClassifier(GeneralDataset<L, F> data, double l1reg, double tol) {
        return trainClassifier(data, l1reg, tol, new LogPrior(LogPrior.LogPriorType.QUADRATIC), false);
    }

    public LogisticRegressionClassifier<L,F> trainClassifier(GeneralDataset<L, F> data, double l1reg, double tol, LogPrior prior) {
        return trainClassifier(data, l1reg, tol, prior, false);
    }

    public LogisticRegressionClassifier<L,F> trainClassifier(GeneralDataset<L, F> data, double l1reg, double tol, boolean biased) {
        return trainClassifier(data, l1reg, tol, new LogPrior(LogPrior.LogPriorType.QUADRATIC), biased);
    }

    public LogisticRegressionClassifier<L,F> trainClassifier(GeneralDataset<L, F> data, double l1reg, double tol, LogPrior prior, boolean biased) {
        if(data instanceof RVFDataset)
            ((RVFDataset<L,F>)data).ensureRealValues();
        if (data.labelIndex.size() != 2) {
            throw new RuntimeException("LogisticClassifier is only for binary classification!");
        ...
        classes[0] = data.labelIndex.get(0);
        classes[1] = data.labelIndex.get(1);
        return new LogisticRegressionClassifier<>(weights, featureIndex, classes);
    }
